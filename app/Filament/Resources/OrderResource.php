...
class OrderResource extends Resource
{
        ...
        return $form->schema([
            \Filament\Forms\Components\Placeholder::make('items_info')
                ...
                ->content(function ($record) {
                    if (!$record) {
                        return 'Produk yang dipesan akan ditampilkan di sini setelah pesanan dibuat.';
                    }
            ...
            Textarea::make('alamat_lengkap')
                ...
                ->formatStateUsing(function ($state, $record) {
                    return $record ? $record->alamat . ', ' . $record->kecamatan . ', ' . $record->kota . ', ' . $record->provinsi . '. ' . $record->kode_pos : '';
                })
                ->columnSpanFull(),
            ...
            ->label('Tolak Pesanan')
            ->reactive()
            ->visible(fn ($record) => $record && $record->status === 'Menunggu Verifikasi')
            ->helperText('Jika dicentang, pesanan ini akan ditolak.'), // 🟢 Ubah teksnya
        ...
        Select::make('status')
            ...
            ->options(function ($get, $livewire) {
                ...
                return match ($originalStatus) {
                    'Menunggu Pembayaran' => [ 'Menunggu Pembayaran' => 'Menunggu Pembayaran' ],
                    'Menunggu Verifikasi' => [ 'Menunggu Verifikasi' => 'Menunggu Verifikasi', 'Diproses' => 'Diproses', 'Ditolak' => 'Ditolak' ], // 🟢 Tambahkan opsi Ditolak
                    'Diproses'            => [ 'Diproses' => 'Diproses', 'Dikirim' => 'Dikirim' ],
                    'Dikirim'             => [ 'Dikirim' => 'Dikirim', 'Selesai' => 'Selesai' ],
                    'Selesai'             => [ 'Selesai' => 'Selesai' ],
                    'Ditolak'             => [ 'Ditolak' => 'Ditolak' ], // 🟢 Tambahkan ini
                    default               => [
                ...
                ->disabled(function ($get, $livewire) {
                    $record = method_exists($livewire, 'getRecord') ? $livewire->getRecord() : null;
                    return !$record || in_array($record->status, ['Menunggu Pembayaran', 'Selesai']) || $get('tolak_pesanan');
                })
                ->columnSpanFull(),
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('no_resi')
                    ->label('Nomor Resi')
                    ->formatStateUsing(fn ($state, $record) => $record?->status === 'Dikirim' ? $state : null)
                    ->placeholder('belum dikirim'),
                
                // Kolom untuk gambar resi
                // IconColumn::make('receipt_image')
                //     ->label('Gambar Resi')
                //     ->icon(fn ($record) => $record?->receipt_image ? 'heroicon-o-document-arrow-down' : 'heroicon-o-x-circle')
                //     ->color(fn ($record) => $record?->receipt_image ? 'success' : 'danger')
                //     ->url(fn ($record) => $record?->receipt_image ? asset('storage/' . $record->receipt_image) : '#')
                //     ->openUrlInNewTab()
    ...
    public static function boot()
    {
        ...
        // 🟢 FUNGSI BARU: Hapus order yang kadaluarsa (lebih dari 1 hari)
        // Ini akan berjalan setiap kali halaman order dimuat.
        try {
            ...
            foreach ($expiredOrders as $order) {
                // Hapus item-item yang terkait dengan order ini untuk menghindari data yatim piatu
                $order->items()->delete();
                // Hapus order itu sendiri
                $order->delete();
            }
        } catch (\Exception $e) {
            // Log error jika terjadi masalah saat menghapus (opsional)
            Log::error("Gagal menghapus order yang kadaluarsa: " . $e->getMessage());
        }

        static::updated(function (Order $order) {
            if ($order->isDirty('status') && $order->status === 'Diproses' && !$order->receipt_image) {
                $receiptHtml = view('receipt', ['order' => $order])->render();
                
                $fileName = 'receipts/' . $order->order_number . '.png';
                
                \Spatie\Browsershot\Browsershot::html($receiptHtml)
                    // Ganti jalur ini dengan jalur yang Anda temukan
